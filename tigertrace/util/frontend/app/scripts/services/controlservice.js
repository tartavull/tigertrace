...
angular.module('cubeApp')
  .service('controlService', function (meshService, tileService, planeService,
    ...
    function init() {
      ...
      tileService.suscribe_loaded(function(){
        sendHumanDecision('');
      });
    };
    init();
    ...
    function mousewheel( event ) {
      ...
      tileDelta(event.wheelDelta / 80);
      srv.needsRender = true;
    }
    document.addEventListener('mousewheel', mousewheel, false);

    function tileDelta(delta) {
      tileService.currentTileFloat = clamp(tileService.currentTileFloat + delta/2.0, 0, globals.CUBE_SIZE.z - 1);
    ...
    function removeAllVisibleMeshes() {
    ...
    function sendHumanDecision( decision ) {
      removeAllVisibleMeshes();
        taskService.submitEdgeDecision(decision, function(edge) {
          next();
        });
      tileService.draw()
    }
    ...
    function checkForTileClick(event) {
      ...
      if (intersects.length) {
        ...
        console.log('Clicked on segment:' + segment_id.toString());
